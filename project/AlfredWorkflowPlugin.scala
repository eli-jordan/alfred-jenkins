...
object AlfredWorkflowPlugin extends AutoPlugin {
  ...
  override lazy val projectSettings: Seq[Setting[_]] = Seq(
    Zero / alfredWorkflowDir := baseDirectory.value / "alfred",
    Zero / alfredWorkflowVersion := version.value,
    Zero / alfredWorkflowExtraFiles := Seq.empty,
    Zero / alfredWorkflowVariables := Map.empty,
    Zero / alfredWorkflowStage := {
    ...
    Zero / alfredWorkflowPackage := {
      val outDir     = target.value / "alfred"
      val stagingDir = alfredWorkflowStage.value
      packageWorkflow(stagingDir, alfredWorkflowName.value, outDir)
    }
  )
  ...
  def processPlist(
    ...
    val nonExported = properties
      .get("variablesdontexport")
      .asInstanceOf[NSArray]
      .getArray
      .toList
